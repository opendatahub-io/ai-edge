...
model from the `model-workspace`, which can be set to the PVC created in the last step, to the `buildah-cache`
workspace, which is then used by the `buildah` task in the
pipeline.[copy-model-from-pvc](pipelines/tekton/aiedge-e2e/tasks/copy-model-from-pvc.yaml) task is not included in the
pipeline by default, you have to add it yourself.
